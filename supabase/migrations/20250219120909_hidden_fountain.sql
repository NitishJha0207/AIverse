/*
  ...
  1. New Tables
    - `app_installations`
      - Tracks app installation status and progress
      - Stores installation history and metadata
      - Links users with their installed apps
  ...
  3. Indexes
    - Optimize queries for installation status checks
    - Improve performance for user-specific queries
*/
...
CREATE INDEX idx_app_installations_created_at ON app_installations(created_at DESC);

-- Create function to update timestamps
CREATE FUNCTION update_app_installation_timestamp()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
-- Create indexes
CREATE INDEX idx_installation_logs_installation_id 
  ON installation_logs(installation_id);
CREATE INDEX idx_installation_logs_created_at 
  ON installation_logs(created_at DESC);

-- Function to log installation events
CREATE FUNCTION log_installation_event()
RETURNS TRIGGER AS $$
BEGIN
...
-- Create trigger for logging installation events
